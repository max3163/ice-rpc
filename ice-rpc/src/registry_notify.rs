...
fn get_or_create_notifier() -> Result<
...
> {
    ...
    let node = ServiceLocator::global().get_node_sync().map_err(|e| {
        log::warn!("[notify] get_node_sync failed (shutdown?): {}", e);
    })?;
    let topic_name = ServiceName::new(REGISTRY_NOTIFY_TOPIC).map_err(|e| {
    ...
    let notifier = svc.notifier_builder().create().map_err(|e| {
...
pub fn notify_change(node_id: u32) {
    let notifier = try_or_log!(get_or_create_notifier(), "get_or_create_notifier", "failed");
    if let Err(e) = notifier.notify_with_custom_event_id(EventId::new(node_id as usize)) {
        log::error!("notify_with_custom_event_id failed: {:?}", e);
    }
}
