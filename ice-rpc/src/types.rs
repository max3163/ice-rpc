...
//! - The stream events ([`Event`], [`ObservableError`]) and the [`Observable`] type
...
/// The Rx observable: a stream of values that may complete or error.
///
/// This is both the return type of methods generated by `#[service]` and the
/// composable stream type. It is an alias of [`Stream`], because the concrete
/// type is the push→pull boundary of the transport.
pub type Observable<T, E> = Stream<T, E>;
...
impl<T, E> Sender<T, E> {
    ...
    ///
    /// This is the only way a technical error transits; producers emit it only
    /// through [`Sender::technical_error`].
    #[inline]
    ...
    /// Alias for [`Sender::send_next`] (Rx naming).
    #[inline]
    pub async fn next(&self, value: T) -> Result<(), async_channel::SendError<WireEvent<T, E>>> {
        self.send_next(value).await
    }

    /// Alias for [`Sender::send_complete`] (Rx naming).
    #[inline]
    pub async fn complete(&self) -> Result<(), async_channel::SendError<WireEvent<T, E>>> {
        self.send_complete().await
    }

    /// Alias for [`Sender::send_error`]: emits a **business** error (Rx naming).
    #[inline]
    pub async fn error(&self, err: E) -> Result<(), async_channel::SendError<WireEvent<T, E>>> {
        self.send_error(err).await
    }

    /// Emits a **technical** error (framework/transport failure).
    #[inline]
    pub async fn technical_error(
        &self,
        err: RpcError,
    ) -> Result<(), async_channel::SendError<WireEvent<T, E>>> {
        self.inner.send(WireEvent::RpcError(err)).await
    }
...
pin_project_lite::pin_project! {
    /// Internal storage backing a [`Stream`].
    ///
    ...
    enum StreamInner<T, E> {
        ...
        Boxed {
            ...
            // `Stream` is not, since its `Channel` variant holds a `Receiver`).
...
pin_project_lite::pin_project! {
    ...
    ///
    /// Cloning a channel-backed stream yields another handle on the *same* channel
    /// (competing consumers); cloning a buffered stream **copies** the remaining
    /// events (each handle then drains its own queue). A stream built with
    /// [`Stream::from_stream`] **cannot** be cloned.
...
impl<T: Clone, E: Clone> Clone for Stream<T, E> {
    fn clone(&self) -> Self {
        let inner = match &self.inner {
            ...
            StreamInner::Boxed { .. } => panic!(
                "a stream built with `Stream::from_stream` cannot be cloned: \
                 rebuild it from its constructor instead"
            ),
        };
        Self { inner }
    }
...
impl<T, E> Stream<T, E> {
    /// Builds a pure, channel-free stream from an already-computed event list.
    ///
    /// Used by the `of`/`from` constructors and by
    /// [`Stream::from_technical_error`]. The events are stored in an inline
    ...
    /// Builds a stream that only emits a terminal technical error.
    ///
    ...
    /// Builds a [`Stream`] from any stream of [`Event`].
    ///
    /// This is what makes an **operator pipeline** usable as the return value of
    /// a service method: a `Stream` must satisfy the generated proxy, which has
    /// a single return type for both its `Provider` (in-process implementation)
    /// and `Consumer` (IPC client) modes, so an operator type (`Map<…>`,
    /// `Delay<…>`, …) cannot be returned directly.
    ///
    ...
    ///
    /// Takes `&mut self` because a buffered stream drains an inline queue
            ...
            StreamInner::Channel { rx, pending } => {
                ...
                match rx.recv().await {
                    Ok(WireEvent::CompleteWith(v)) => {
                        store_pending(pending, Event::Complete);
                        Ok(Event::Next(v))
                    }
                    Ok(WireEvent::Next(v)) => Ok(Event::Next(v)),
                    Ok(WireEvent::Complete) => Ok(Event::Complete),
                    Ok(WireEvent::Error(e)) => Ok(Event::Error(ObservableError::Business(e))),
                    Ok(WireEvent::RpcError(e)) => Ok(Event::Error(ObservableError::Technical(e))),
                    Err(e) => Err(e),
                }
            }
    ...
    ///
    /// For a channel-backed stream this is a plain passthrough of the producer
    /// [`WireEvent`]. For a buffered stream (built by [`Stream::from_events`],
    /// i.e. `of` / `from` / `throw_error`) the last `Next(v)` followed by a
    /// `Complete` is folded back into the single-sample
    ...
    pub async fn recv_wire(&mut self) -> Result<WireEvent<T, E>, async_channel::RecvError> {
        match &mut self.inner {
            ...
            StreamInner::Boxed { stream, pending } => {
                ...
                futures_lite::future::poll_fn(|cx| {
                    match futures_lite::Stream::poll_next(Pin::new(&mut *stream), cx) {
                        ...
                        Poll::Ready(Some(other)) => Poll::Ready(Ok(to_wire_event(other))),
            ...
            StreamInner::Buffered { queue } => match queue.pop_front() {
                ...
                Some(Event::Complete) => Ok(WireEvent::Complete),
                Some(Event::Error(ObservableError::Business(e))) => Ok(WireEvent::Error(e)),
                Some(Event::Error(ObservableError::Technical(e))) => Ok(WireEvent::RpcError(e)),
            },
        }
    }
    ...
    pub async fn first_value(mut self) -> Result<T, StreamError<E>> {
    ...
    /// returned.
    pub async fn collect(mut self) -> Result<Vec<T>, ObservableError<E>> {
...
fn store_pending<T, E>(
    ...
    *pending.lock().expect("stream pending lock poisoning") = Some(event);
}
        ...
        match self.project().inner.project() {
            StreamInnerProj::Channel { rx, pending } => {
                ...
                match futures_lite::Stream::poll_next(rx, cx) {
                    ...
                    Poll::Ready(Some(WireEvent::Error(e))) => {
                        Poll::Ready(Some(Event::Error(ObservableError::Business(e))))
                    }
                    Poll::Ready(Some(WireEvent::RpcError(e))) => {
                        Poll::Ready(Some(Event::Error(ObservableError::Technical(e))))
                    }
...
fn to_wire_event<T, E>(event: Event<T, E>) -> WireEvent<T, E> {
    match event {
        Event::Next(v) => WireEvent::Next(v),
        Event::Complete => WireEvent::Complete,
        Event::Error(ObservableError::Business(e)) => WireEvent::Error(e),
...
/// Error returned by [`Stream::first_value`].
///
...
/// transport boundary: producers push, the returned [`Stream`] pulls.
...
mod tests {
    ...
    fn event_is_terminal_flags() {
        ...
        assert!(
            Event::<i32, String>::Error(ObservableError::Business("boom".to_string()))
                .is_terminal()
        );
        assert!(
            Event::<i32, String>::Error(ObservableError::Technical(RpcError::Timeout))
                .is_terminal()
        );
    }
    ...
    fn from_events_is_channel_free_and_replays_in_order() {
        let mut stream =
            Stream::<i32, String>::from_events([Event::Next(1), Event::Next(2), Event::Complete]);

        assert!(!matches!(stream.inner, StreamInner::Channel { .. }));
    ...
    fn buffered_recv_wire_coalesces_next_complete_into_complete_with() {
        let mut stream = Stream::<i32, String>::from_events([Event::Next(42), Event::Complete]);
    ...
    fn buffered_recv_wire_keeps_intermediate_values_then_coalesces_last() {
        let mut stream = Stream::<i32, String>::from_events([
            Event::Next(1),
            Event::Next(2),
            Event::Next(3),
            Event::Complete,
    ...
    fn buffered_recv_wire_maps_terminal_errors() {
        let mut business = Stream::<i32, String>::from_events([Event::Error(
        ...
        let mut technical = Stream::<i32, String>::from_technical_error(RpcError::Timeout);
        assert!(matches!(
    ...
    fn wire_samples<T, E>(mut stream: Stream<T, E>) -> Vec<WireEvent<T, E>> {
        let mut out = Vec::new();
    ...
    fn boxed_stream_delegates_poll_next_and_coalesces() {
        let inner = Stream::<i32, String>::from_events([Event::Next(42), Event::Complete]);
        let stream = Stream::<i32, String>::from_stream(inner);
    ...
    fn boxed_stream_does_not_wait_for_the_terminal_event() {
        ...
        let mut stream = Stream::<i32, String>::from_stream(GatedStream {
            gate: gate.clone(),
            step: 0,
        });
    ...
    fn single_value_source_travels_as_exactly_one_wire_sample() {
        ...
        let samples = wire_samples(Stream::<i32, String>::from_events([
            Event::Next(42),
            Event::Complete,
        ]));
    ...
    fn business_error_source_travels_as_exactly_one_wire_sample() {
        ...
        let samples = wire_samples(Stream::<i32, String>::from_events([Event::Error(
            ObservableError::Business("boom".to_string()),
        )]));
    ...
    fn technical_error_source_travels_as_exactly_one_wire_sample() {
        // Shape produced by `Stream::from_technical_error` and by the generated
        // client when a call fails before the transport stream exists.
        let samples = wire_samples(Stream::<i32, String>::from_technical_error(
            RpcError::Timeout,
        ));
    ...
    fn from_technical_error_emits_single_terminal_error() {
        let mut stream = Stream::<i32, String>::from_technical_error(RpcError::Cancelled);
    ...
    fn first_value_reports_technical_error() {
        let stream = Stream::<i32, String>::from_technical_error(RpcError::Timeout);
    ...
    fn collect_returns_business_error() {
        let stream = Stream::<i32, String>::from_events([
            Event::Next(1),
    ...
    fn buffered_stream_poll_next_yields_none_when_drained() {
        let stream = Stream::<i32, String>::from_events([Event::Next(7)]);
