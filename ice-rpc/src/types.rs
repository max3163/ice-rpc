...
/// Normalizes a wire event into a `next*` + `complete`/`error` sequence.
///
/// A [`Event::CompleteWith`] is replayed as [`Event::Next`] followed by
/// [`Event::Complete`], so that consumers never observe the transport
/// optimization. Other events are returned unchanged.
...
/// Wraps a stream so that every `CompleteWith` is replayed as `Next` + `Complete`.
pub fn normalize_stream<T, E>(stream: Stream<T, E>) -> Stream<T, E>
where
    T: Send + 'static,
    E: Send + 'static,
{
    let (tx, rx) = channel::<T, E>(8);
    crate::rt::spawn(async move {
        while let Ok(event) = stream.recv().await {
            for ev in normalize_event(event) {
...
/// Normalizes the stream of an observable result.
pub fn normalize_observable<T, E>(observable: Observable<T, E>) -> Observable<T, E>
where
    T: Send + 'static,
    E: Send + 'static,
{
    observable.map(normalize_stream)
}
...
mod tests {
    ...
    fn event_is_terminal_flags() {
        ...
        assert!(Event::<i32, String>::CompleteWith(1).is_terminal());
        assert!(Event::<i32, String>::Error("boom".to_string()).is_terminal());
        assert!(Event::<i32, String>::RpcError(RpcError::Timeout).is_terminal());
    }

    #[test]
    fn normalize_event_splits_complete_with() {
        let normalized = normalize_event::<i32, String>(Event::CompleteWith(7));
        assert_eq!(normalized.len(), 2);
        match &normalized[0] {
            Event::Next(v) => assert_eq!(*v, 7),
            other => panic!("expected Next, got {:?}", other),
        }
        assert!(matches!(&normalized[1], Event::Complete));
    }

    #[test]
    fn normalize_event_passes_other_variants_through() {
        let normalized = normalize_event::<i32, String>(Event::Error("e".to_string()));
        assert_eq!(normalized.len(), 1);
        assert!(matches!(&normalized[0], Event::Error(_)));
    }

    #[test]
    fn event_rkyv_roundtrip_complete_with() {
        let event: Event<i32, String> = Event::CompleteWith(42);
        let bytes = rkyv::to_bytes::<rkyv::rancor::Error>(&event).unwrap();
        let decoded: Event<i32, String> =
            rkyv::from_bytes::<_, rkyv::rancor::Error>(&bytes).unwrap();
        match decoded {
            Event::CompleteWith(v) => assert_eq!(v, 42),
